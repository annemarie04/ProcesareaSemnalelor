...
for it_gradient in range(len(gradient_threshold)):
    for it_vot in range(len(voting_threshold)):
        ...
        for it in range(len(imgs)):
            ...
            img = np.array(Image.open(f'{imgs[it]}'))
...
#print(Accumulator[Accumulator>=100])
            ...
            plt.savefig(f'{imgs[it][:-4]}_Hough.png', format='png')
            plt.show()
            plt.figure()
            #%%
            img2 = np.array(Image.open(f'{imgs[it][:-4]}_Hough.png'))
            img2 = cv2.cvtColor(img2[:,:,:3], cv2.COLOR_RGB2GRAY)
            #%%
            plt.imshow(img2, cmap='gray')
            plt.show()
            img3 = np.array(Image.open(f'{correct_imgs[it]}'))
